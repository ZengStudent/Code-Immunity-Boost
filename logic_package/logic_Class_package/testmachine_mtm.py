...
class Testmachine_mtm(Testmachine):
    # initial
    ...
    def __read_testcase(self):
        ...
        originalcode_path = CORE_DIR + '/database_folder/database_contract_folder/contract_' + str(self.contractid) + '/originalcode/'
        # Contract的questioncode路徑
        questioncode_path = CORE_DIR + '/database_folder/database_contract_folder/contract_' + str(self.contractid) + '/questioncode/'
...
'''
        ...
        for i in range(1, self.usercode_count + 1, 1):
            ...
            f.write(import_mutnat + import_unittest + import_path + import_order + self.usercode['usercodes_' + str(i)] +'\n'+ import_finally_code)
            f.seek(0)
            f.close()
        ...
        for name in os.listdir(user_mutant_path):
            ...
            for j in range(1, self.usercode_count + 1):
                ...
                import_mutnat = ff.read() + '\n'
                ff.close()
                import_order = 'order = ' + '\'' + str(j) + '_' + str(name[_position + 1:-3]) + '\'' + '\n'

                file_dir = user_unittest_path + '/unittest_testcase_' + str(j) + '_' + str(name[_position + 1:-3]) + '.py'
                f = open(file_dir, 'w+', encoding='UTF-8')
                # 依照使用者寫入的code，生成單元測試py檔案
                f.write(import_mutnat + import_unittest + import_path + import_order + self.usercode['usercodes_' + str(j)] +'\n'+ import_finally_code)
                f.seek(0)
                f.close()
...
'''
        ...
        for name in os.listdir(user_testcase_path):
            ...
            for j in range(1, self.usercode_count + 1):
                ...
                file_dir = user_unittest_path + '/unittest_testcase_' + str(name[_position + 1:-4]) + '_' + str(j) + '.py'
                f = open(file_dir, 'w+', encoding='UTF-8')
                # 依照使用者寫入的code，生成單元測試py檔案
    ...
    def __do_unittest(self):
        ...
        # Category:Aggression
            ...
            for name in os.listdir(user_unittest_result_path):
                ...
                # 讀取單元測試結果
                user_original_unittest_result_path = CORE_DIR + '/overarea_folder/overarea_User_Folder/' + self.username + '/unittest_result_folder/' + 'unittest_original_' + name[rightposition + 1:-4] + '.txt'
                user_original_unittest_expect_path = CORE_DIR + '/overarea_folder/overarea_User_Folder/' + self.username + '/unittest_result_folder/' + 'unittest_expect_original_' + name[rightposition + 1:-4] + '.txt'
    ...
    def __do_checkcontract_basic_guard_ut(self):
        ...
        # 建立字典
        for index in range(1,self.contract_oppositescount+1):
            local_max_dict[str(index)] = 0
        ...
        for name in os.listdir(user_unittest_path):
            ...
            if(normalresult == originalresult['unittest_original_' + str(name[_leftposition+1:_rightposition])]):
                ...
                local_nokilled['unittest_testcase_' + str(name[_leftposition+1:_rightposition])] = 0
                local_nokilled['unittest_testcase_' + str(name[_leftposition+1:_rightposition])] = local_nokilled['unittest_testcase_' + str(name)[_leftposition+1:_rightposition]] + 1
            ...
            # 結果不同
                ...
                local_killed['unittest_testcase_' + str(name[_leftposition+1:_rightposition])] = 0
                local_killed['unittest_testcase_' + str(name[_leftposition+1:_rightposition])] = local_killed['unittest_testcase_' + str(name[_leftposition+1:_rightposition])] + 1
                ...
                self.__squaredict['unittest_testcase_' + str(name[_leftposition + 1:_rightposition])][str(name[_rightposition + 1:-4])]['mutantcodename'] = str(normalresult)
                self.__squaredict['unittest_testcase_' + str(name[_leftposition + 1:_rightposition])][str(name[_rightposition + 1:-4])]['scorecodename'] = '+++'
                local_max_dict[str(name)[_rightposition + 1:-4]] = local_max_dict[str(name)[_rightposition + 1:-4]] + 1

        # 累計
        for name,value in local_max_dict.items():
            if(value > 0):
        ...
        self.__basicprecent = (local_max / self.contract_oppositescount) * 100.0

        # Small Report
        self.__smallreport['basic'] = '完成度【基本】\n測試案例總共發現變異體 : ' + str(local_max) + ' 個\n'
        self.__smallreport['basic'] = self.__smallreport['basic'] + '本關卡要找出 : ' + str(int(self.contract_oppositescount * (self.contentprecent / 100.0))) + ' 個變異體\n'
        self.__smallreport['basic'] = self.__smallreport['basic'] + '本次得分為 : ' + str(local_max) + ' / ' + str(int(self.contract_oppositescount * (self.contentprecent / 100.0))) + ' = '

        if (self.__basicprecent >= self.contentprecent):
            self.__basicprecent = self.__basicprecent / (self.contentprecent / 100.0)
            self.__smallreport['basic'] = self.__smallreport['basic'] + str((local_max/(self.contract_oppositescount * (self.contentprecent / 100.0)))*100.0) + '(超過100%，視為100%)\n'
    ...
    def __do_checkcontract_basic_guard_ms(self):
        ...
        for name in os.listdir(user_unittest_path):
            ...
            if (normalresult == originalresult['unittest_original_' + str(name[_leftposition + 1:_rightposition])]):
                ...
                local_nokilled['unittest_testcase_' + str(name[_leftposition + 1:_rightposition])] = 0
                local_nokilled['unittest_testcase_' + str(name[_leftposition + 1:_rightposition])] = local_nokilled[
                                                                                                         'unittest_testcase_' + str(
                                                                                                             name)[
            ...
            # 結果不同
                ...
                local_killed['unittest_testcase_' + str(name[_leftposition + 1:_rightposition])] = 0
                local_killed['unittest_testcase_' + str(name[_leftposition + 1:_rightposition])] = local_killed[
                                                                                                       'unittest_testcase_' + str(
                                                                                                           name[
                                                                                                           _leftposition + 1:_rightposition])] + 1
        ...
        # Small Report
        self.__smallreport['basic'] = '完成度【基本】\n測試案例總共殺死變異體 : ' + str(local_max) + ' 個\n'
        self.__smallreport['basic'] = self.__smallreport['basic'] + '本關卡要變異分數達 : '  + str(self.contentprecent) + '%'  + '\n'
        self.__smallreport['basic'] = self.__smallreport['basic'] + '本次變異分數為 : ' + str(local_max) + ' / ' + str(self.contract_oppositescount)  + ' = '


        if (self.__basicprecent >= self.contentprecent):
            self.__basicprecent = self.__basicprecent / (self.contentprecent / 100.0)
            self.__smallreport['basic'] = self.__smallreport['basic'] + str((local_max / self.contract_oppositescount) * 100.0) + '%' + '\n'
            self.__smallreport['basic'] = self.__smallreport['basic'] + '該關卡完成度【基本】為 ' + str((local_max / (self.contract_oppositescount * (self.contentprecent / 100.0))) * 100.0) + '(超過100%，視為100%)\n'
            self.__basiccompletescore['score'] = str((local_max / self.contract_oppositescount) * 100.0) + '%'
            self.__completeprecent = self.__basicprecent / 2.0
            self.__completeprecent = ((local_max / self.contract_oppositescount) * 100.0)
    ...
    def __do_checkcontract_basic_Aggression_ut(self):
        ...
        for name in os.listdir(user_unittest_path):
            ...
            # 結果相同
            if (normalresult == originalresult['unittest_original_' + str(name[_leftposition + 1:_rightposition])]):
                local_nokilled['unittest_testcase_' + str(name[_leftposition + 1])] = 0
                local_nokilled['unittest_testcase_' + str(name[_leftposition+1])] = local_nokilled['unittest_testcase_' + str(name[_leftposition+1])] + 1
                ...
                self.__squaredict['unittest_mutant_' + str(name)[_rightposition + 1:-4]][str(name)[_leftposition + 1:_rightposition]]['usercodenumber'] = 'Testcase ' + str(name)[_leftposition + 1:_rightposition]
                ...
                local_max_dict[str(name)[_leftposition + 1:_rightposition]] = local_max_dict[str(name)[_leftposition + 1:_rightposition]] + 1


            else:
                # 結果不同
                local_killed['unittest_testcase_' + str(name[_leftposition + 1])] = 0
                local_killed['unittest_testcase_' + str(name[_leftposition+1])] = local_killed['unittest_testcase_' + str(name[_leftposition+1])] + 1
        ...
        self.__basicprecent = (local_max / self.usercode_count) * 100.0

        # Small Report
        self.__smallreport['basic'] = '完成度【基本】\n變異體總共沒有被 : ' + str(local_max) + ' 個測試案例發現錯誤\n'
        self.__smallreport['basic'] = self.__smallreport['basic'] + '本關卡要不被 : ' + str(int(self.contract_oppositescount * (self.contentprecent / 100.0))) + ' 個測試案例發現錯誤\n'
        self.__smallreport['basic'] = self.__smallreport['basic'] + '本次得分為 : ' + str(local_max) + ' / ' + str(int(self.contract_oppositescount * (self.contentprecent / 100.0))) + ' = '
    ...
    def __do_checkcontract_basic_Aggression_ms(self):
        ...
        for name in os.listdir(user_unittest_path):
            ...
            if("unittest_10" in name):
                print('\n------\n')
                print(normalresult)
                print('\n------\n')
                print(originalresult['unittest_original_' + str(name[_leftposition + 1:_rightposition])])
        ...
        # Small Report
        self.__smallreport['basic'] = '完成度【基本】\n總共有變異體 : ' + str(self.contract_oppositescount - local_max) + ' 個被測試案例發現錯誤\n'
        self.__smallreport['basic'] = self.__smallreport['basic'] + '本關卡脆弱值要低於 : ' + str(self.contentprecent/100.0) +  '\n'
        self.__smallreport['basic'] = self.__smallreport['basic'] + '本次脆弱值為 : ' + str(self.contract_oppositescount - local_max) + ' / ' + str(self.contract_oppositescount)  + ' = '

        if (self.__basicprecent >= self.contentprecent):
            self.__basicprecent = self.__basicprecent / (self.contentprecent / 100.0)
            self.__smallreport['basic'] = self.__smallreport['basic'] + str(((self.contract_oppositescount - local_max)/self.contract_oppositescount) * 100.0) + '\n'
            self.__basiccompletescore['score'] = str(((self.contract_oppositescount - local_max)/self.contract_oppositescount) * 100.0)
            try :
                self.__smallreport['basic'] = self.__smallreport['basic'] + '該關卡完成度【基本】為 ' + str((self.contentprecent/((self.contract_oppositescount - local_max)/self.contract_oppositescount))) + '(超過100%，視為100%)\n'
            except ZeroDivisionError:
                self.__smallreport['basic'] = self.__smallreport['basic'] + '該關卡完成度【基本】為 ' + str((self.contentprecent/1.0)*100.0) + '(超過100%，視為100%)\n'
            self.__completeprecent = (((self.contract_oppositescount - local_max)/self.contract_oppositescount) * 100.0)
            if (self.__basicprecent >= 100.0):
                self.__basicprecent = 100.0
            self.__checkbasic = True
            # 通過基本條件分數
    ...
    def __do_checkcontract_advance_guard_ut(self):
        ...
        self.__advanceprecent = (local_max / self.contract_oppositescount ) * 100.0

        # Small Report
        self.__smallreport['advance'] = '\n\n完成度【進階】\n測試案例總共發現變異體 : ' + str(local_max) + ' 個\n'
        self.__smallreport['advance'] = self.__smallreport['advance'] + '本關卡要找出 : ' + str(int(self.contract_oppositescount * (self.perfectprecent / 100.0))) + ' 個變異體\n'
        self.__smallreport['advance'] = self.__smallreport['advance'] + '本次得分為 : ' + str(local_max) + ' / ' + str(int(self.contract_oppositescount * (self.perfectprecent / 100.0))) + ' = '

        if (self.__advanceprecent >= self.perfectprecent):
            self.__advanceprecent = self.__advanceprecent / (self.perfectprecent / 100.0)
            self.__smallreport['advance'] = self.__smallreport['advance'] + str((local_max / (self.contract_oppositescount * (self.perfectprecent / 100.0)))*100.0) + '(超過100%，視為100%)\n'
    ...
    def __do_checkcontract_advance_guard_ms(self):
        ...
        self.__smallreport['advance'] = self.__smallreport['advance'] + '本關卡要變異分數達 : ' + str(self.perfectprecent) + '%' + '\n'
        self.__smallreport['advance'] = self.__smallreport['advance'] + '本次變異分數為 : ' + str(local_max) + ' / ' + str(self.contract_oppositescount)  + ' = '

        if (self.__advanceprecent >= self.perfectprecent):
            self.__advanceprecent = self.__advanceprecent / (self.perfectprecent / 100.0)
            self.__smallreport['advance'] = self.__smallreport['advance'] + str( (local_max / self.contract_oppositescount) * 100.0) + '%' + '\n'
            self.__smallreport['advance'] = self.__smallreport['advance'] + '該關卡完成度【進階】為 ' + str((local_max / (self.contract_oppositescount * (self.perfectprecent / 100.0))) * 100.0) + '(超過100%，視為100%)\n'
            self.__advancecompletescore['score'] = str( (local_max / self.contract_oppositescount) * 100.0) + '%'
            self.__completeprecent = self.__completeprecent + (self.__advanceprecent / 2.0)
            self.__completeprecent = ((local_max / self.contract_oppositescount) * 100.0)
    ...
    def __do_checkcontract_advance_Aggression_ut(self):
        ...
        # Small Report
        self.__smallreport['advance'] = '\n\n完成度【進階】\n變異體總共沒有被 : ' + str(local_max) + ' 個測試案例發現錯誤\n'
        self.__smallreport['advance'] = self.__smallreport['advance'] + '本關卡要不被 : ' + str(int(self.contract_oppositescount * (self.perfectprecent / 100.0))) + ' 個測試案例發現錯誤\n'
        self.__smallreport['advance'] = self.__smallreport['advance'] + '本次得分為 : ' + str(local_max) + ' / ' + str(int(self.contract_oppositescount * (self.perfectprecent / 100.0))) + ' = '
        ...
        if (self.__advanceprecent >= self.perfectprecent):
            ...
            self.__advanceprecent = self.__advanceprecent / (self.perfectprecent / 100.0)
            self.__smallreport['advance'] = self.__smallreport['advance'] + str(self.__advanceprecent) + '(超過100%，視為100%)\n'
            self.__checkadvance = False
            print('Advance Fail.')
            return False
    ...
    def __do_checkcontract_advance_Aggression_ms(self):
        ...
        for name in os.listdir(user_unittest_path):
            ...
            if (normalresult == originalresult['unittest_original_' + str(name[_leftposition + 1:_rightposition])]):
                local_nokilled['unittest_testcase_' + str(name[_leftposition + 1])] = 0
                local_nokilled['unittest_testcase_' + str(name[_leftposition + 1])] = local_nokilled[
                                                                                          'unittest_testcase_' + str(
                                                                                              name[
                                                                                                  _leftposition + 1])] + 1
                local_max_dict[str(name)[_leftposition + 1:_rightposition]] = local_max_dict[str(name)[
                                                                                             _leftposition + 1:_rightposition]] + 1
                ...
                # 結果不同
                local_killed['unittest_testcase_' + str(name[_leftposition + 1])] = 0
                local_killed['unittest_testcase_' + str(name[_leftposition + 1])] = local_killed[
                                                                                        'unittest_testcase_' + str(name[
                                                                                                                       _leftposition + 1])] + 1
        ...
        # Small Report
        self.__smallreport['advance'] = '完成度【進階】\n總共有變異體 : ' + str(self.contract_oppositescount - local_max) + ' 個被測試案例發現錯誤\n'
        self.__smallreport['advance'] = self.__smallreport['advance'] + '本關卡脆弱值要低於 : ' + str(self.perfectprecent/100.0) +  '\n'
        self.__smallreport['advance'] = self.__smallreport['advance'] + '本次脆弱值為 : ' + str(self.contract_oppositescount - local_max) + ' / ' + str(self.contract_oppositescount)  + ' = '

        if (self.__advanceprecent >= self.perfectprecent):
            self.__advanceprecent = self.__advanceprecent / (self.perfectprecent / 100.0)
            self.__smallreport['advance'] = self.__smallreport['advance'] + str(((self.contract_oppositescount - local_max) / self.contract_oppositescount)*100.0)  + '\n'
            self.__advancecompletescore['score'] = str(((self.contract_oppositescount - local_max) / self.contract_oppositescount)*100.0)
            try :
                self.__smallreport['advance'] = self.__smallreport['advance'] + '該關卡完成度【進階】為 ' + str((self.perfectprecent/((self.contract_oppositescount - local_max)/self.contract_oppositescount))) + '(超過100%，視為100%)\n'
            except ZeroDivisionError:
