...
def guard_otm(request, contractid):
    print(request.user)
    ...
    if request.POST:
        ...
        if 'codesubmit' in request.POST:
            ...
            return render(request, 'guard/guard_page_otm.html', context=l)
        # 使用者回復預設Code
        ...
        # 完成Contract
            ...
            temp_context['username'] = request.user
    ...
    return render(request, 'guard/guard_page_otm.html', context=s)

# ===========================================
# Guard，Many to Many
# ===========================================
@login_required(login_url='/login/')
def guard_mtm(request, contractid):
    print(request.user)
    s = {}
    # 從資料庫讀取題目相關資料
    questioncode = lb.read_questioncode(
    ...
    if request.POST:
        ...
        if 'codesubmit' in request.POST:
            ...
            return render(request, 'guard/guard_page_mtm.html', context=l)
        # 完成Contract
        elif 'codecomplete' in request.POST:
    ...
    return render(request, 'guard/guard_page_mtm.html', context=s)

# ===========================================
# Guard，Many to One
# ===========================================
@login_required(login_url='/login/')
def guard_mto(request, contractid):
    s = {}
    # 從資料庫讀取題目相關資料
    questioncode = lb.read_questioncode(
        CORE_DIR + '/database_folder/database_contract_folder/contract_' + str(contractid) + '/questioncode/')
    ...
    if request.POST:
        ...
        if 'codesubmit' in request.POST:
            ...
            return render(request, 'guard/guard_page_mto.html', context=l)
        # 完成Contract
        elif 'codecomplete' in request.POST:
            user = normaluser.objects.get(author=request.user)
            user.add_correct_counts()
    ...
    return render(request, 'guard/guard_page_mto.html', context=s)
