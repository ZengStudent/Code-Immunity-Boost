...
//副程式 : Initial Editor
//==================================
function aggression_Initial_Editor(editor_textarea,defaulutcode) {
    ...
    for (let i = 1; i < breadpagemax; i++) {
        editor_cm_many[i] = defaulutcode;
    }

    //
    template_code = defaulutcode;
...
function aggression_default(code) {
    ...
    if (typeof(code) === "string") {
        ...
        editor_cm.setValue(code["usercode_1"]);
        //
        editor_cm_many[0] = code["usercode_1"];
        //
        let breadhiddeninput_1 = document.getElementById("breadhiddeninput_1");
        breadhiddeninput_1.value = code["usercode_1"];
        ...
        for (var prop in code) {
            ...
            editor_cm_many[count-1] = code["usercode_" + String(count)];
            aggression_editor_increase();
            count = count + 1;
        }
...
function aggression_rewrite(code) {
    ...
    editor_cm_many[now_breadpage-1] = code;

    //
    let breadhiddeninput = document.getElementById("breadhiddeninput_" + String(now_breadpage));
    breadhiddeninput.value = code;

}


//==================================
//副程式 : Editor 覆蓋內容(Submit)
//==================================
function aggression_rewrite_submit(code) {
    let count = 1;
    now_breadpage = 1;
    if (typeof(code) === "string") {
        editor_cm.setValue(code);
...
//副程式 : 新增Editor
//==================================
function aggression_editor_increase() {
    //Editor Bread
    ...
    if (bread.childElementCount < breadpagemax) {
        ...
        //
        breadhiddeninput.value = editor_cm_many[breadcount-1];
        //
...
//刪除當前顯示頁數的Editor
//==================================
function aggression_editor_decrease() {
    //Editor Increase Button
    let editorincreasebutton = document.getElementById("editorincrease");
    //Editor Decrease Button
    let editordecreasebutton = document.getElementById("editordecrease");
    ...
    if (now_breadpage === 1) {
        ...
        if (bread.childElementCount === 2) {
            ...
            //第2個Editor內容，改變為預設值
            editor_cm_many[1] = template_code;
            //現在Editor的頁數改為第一頁
            now_breadpage = 1;

            //Editor 覆蓋內容，改變為第1個Editor內容
            aggression_rewrite(editor_cm_many[0]);
            //
        ...
        //3(含)個Editor以上
            ...
            for (let i = 1; i <= bread.childElementCount; i++) {
                ...
                if (i === bread.childElementCount) {
                    //最後一個Editor改為預設值
                    editor_cm_many[i] = template_code;
                }
            }
    ...
    //刪除最後一個
        ...
        //
        editor_cm_many[now_breadpage - 1] = template_code;
        //現在Editor的頁數改為現在Editor的頁數的前一頁
        now_breadpage = now_breadpage - 1;
        //Editor覆蓋內容，改變為前1個Editor內容
        aggression_rewrite(editor_cm_many[now_breadpage - 1]);
        //將現在的頁數Button設為不可用
...
//副程式 : Save Editor
//==================================
function aggression_editor_save() {
